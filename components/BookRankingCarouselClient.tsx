...
import type { RankedBook } from "@/lib/bookRanking";

type Shop = "amazon" | "rakuten";

export default function BookRankingCarouselClient({
  books,
}: {
  books: RankedBook[];
}) {
    ...
    <div className="max-w-5xl mx-auto px-4">
      ...
      <div className="flex gap-4 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-thin">
        {books.map((book, i) => (
          ...
          >
            <div className="w-full aspect-[3/4] bg-washi flex items-center justify-center rounded mb-2 relative overflow-hidden">
              <span className="absolute top-1 left-1 z-10 text-[10px] font-bold bg-yamabuki-deep text-white rounded-full w-5 h-5 flex items-center justify-center">
                {i + 1}
              </span>
              {book.imageUrl ? (
      ...
      <p className="text-[10px] text-ink-soft mt-3">
        PR / 楽天市場の売れ筋ランキングを定期的に取得しています。価格は変動する場合があるため、購入前に各ストアでご確認ください。
      </p>
    </div>
  );
}
