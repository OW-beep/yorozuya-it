...
export default function AffiliateBannerClient({
  item,
  rakutenItem,
}: {
  item: AffiliateItem;
  rakutenItem: RakutenBannerItem | null;
}) {
  if (!item) return null;

  // 楽天の実データが取れていれば楽天をデフォルト、取れていなければAmazonのみ
  const [shop, setShop] = useState<Shop>(rakutenItem ? "rakuten" : "amazon");

  const showRakuten = shop === "rakuten" && rakutenItem;
  const displayName = showRakuten ? rakutenItem!.name : item.name;
  const displayUrl = showRakuten ? rakutenItem!.url : withTag(item.url);
  const displayImage = showRakuten ? rakutenItem!.imageUrl : item.image;
  const displayPrice =
    showRakuten && rakutenItem!.price
      ? `¥${rakutenItem!.price.toLocaleString()}`
      : undefined;

  return (
    <div className="not-prose my-8 max-w-md">
      {rakutenItem && (
        <div className="inline-flex border border-ink/20 rounded-full overflow-hidden text-[10px] mb-1.5">
          <button
            type="button"
      ...
      <a
        ...
        className="flex items-center gap-4 border border-ink/15 bg-white rounded-xl px-4 py-4 shadow-sm hover:shadow-md hover:border-yamabuki-deep/50 transition-all group"
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          {displayPrice ? (
            <p className="text-[11px] text-ink-soft mt-0.5">{displayPrice}</p>
          ) : (
            item.note && (
